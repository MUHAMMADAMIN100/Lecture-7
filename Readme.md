...
# New Map()
## Вы можете создать карту, передав массив конструктору
![alt text](image-21.png)
# Метод set()
## Вы можете добавлять элементы на карту
![alt text](image-22.png)
+ Этот set() метод также можно использовать для изменения существующих значений карты: 

![alt text](image-23.png)
# Метод get()
## Метод get()получает значение ключа в карте:
...
# Метод Set
## Set — это коллекция уникальных значений. Каждое значение может встречаться в наборе только один раз. Значения могут быть любого типа, примитивными значениями или объектами.

##  В отличие от массивов, элементы Set не имеют индексов для доступа.
## Set полезен, когда вам нужно работать с коллекцией уникальных элементов, например, для удаления дубликатов из массива или для отслеживания уникальных значений.
...
# Mетод add
## Mетод add используется для добавления элементов в набор.
# Пример 
![alt text](image-30.png)
#  Mетод has
## Используется для того,чтобы проверить, содержится ли элемент в наборе.Возвращает булевое значение
# Пример 1
...
# Метод clear
## Метод clear используется для очистки всех элементов в наборе
# Пример
...
# Метод forEach
## Метод forEach -выводит каждый элемент набора 
# Пример
![alt text](image-37.png)
# Метод entries()
## возвращает новый объект итератора, содержащий массивы [значение, значение] для каждого элемента 
![alt text](image-38.png)  
# 

+ ### В отличие от объектов Map, где используются пары [ключ, значение], Set использует пары [значение, значение] потому, что каждое значение в Set само по себе уникально и не связано с каким-либо ключом.  
